// $Id$
/*
 * This demo application allows you to browse through the root directory
 * of a SD card
 *
 * ==========================================================================
 *
...
void DIN_NotifyToggle(unsigned char pin, unsigned char pin_value) __wparam
{
  ...
  switch( pin ) {
    ...
    case DIN_NUMBER_INC: // Inc button
      // with forward search
      error_code = ReadDir(dir_entry+1, (0 << SDCARD_FILE_MODE_BACKWARD_SEARCH));
      break;

    case DIN_NUMBER_DEC: // Dec button
      if( dir_entry ) {
	// with backward search
	error_code = ReadDir(dir_entry-1, (1 << SDCARD_FILE_MODE_BACKWARD_SEARCH));
      }
      break;
