...
#else
// SDCC doesn't allow to initialize union types
// Workaround:
typedef struct {
  unsigned c:4;
  unsigned cd:4;
...
typedef struct {
  unsigned char notes[6]; // unfortunately "unsigned notes:4[12]" doesn't work
} seq_scale_entry_notes_t;
#endif
...
// Initialisation
/////////////////////////////////////////////////////////////////////////////
char SCALE_Init(unsigned char mode) __wparam
{
...
/////////////////////////////////////////////////////////////////////////////
unsigned char SCALE_NumGet(void) __wparam
{
...
/////////////////////////////////////////////////////////////////////////////
char *SCALE_NameGet(unsigned char scale) __wparam
{
...
// returns < 0 on errors
/////////////////////////////////////////////////////////////////////////////
char SCALE_Note(unsigned char *note, unsigned char scale, unsigned char root) __wparam
{
  unsigned char tmp;
  unsigned char note_scaled;
  const seq_scale_entry_notes_t *table_entry;

  // normalize note and determine octave
#if 0
  char note_number = *note - root;
  char octave = note_number / 12;
  if( note_number < 0 ) note_number += 12;
  note_number = note_number % 12;
#else
  // this iterative method will work faster on a PIC (without hardware division)
  char note_number = *note - root;
  char octave = 0;
  if( note_number < 0 )
    note_number += 12;
  ...
  table_entry = (seq_scale_entry_notes_t *)&seq_scale_table[scale];
  tmp = table_entry->notes[note_number>>1];
  note_scaled = (note_number & 1) ? (tmp >> 4) : (tmp & 0xf);
