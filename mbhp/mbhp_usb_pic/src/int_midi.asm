...
INT_MIDI_Parse_SendPacket
	;; nothing more to do here (we can regognize if a package
...
;; --------------------------------------------------------------------------
INT_MIDI_Receive
	clrf	_int_midi_rx_package+0	; clear package content
	clrf	_int_midi_rx_package+1
	clrf	_int_midi_rx_package+2
	clrf	_int_midi_rx_package+3

	call	MIDI_RxBufferUsed	; exit if nothing in receive buffer
	bz	INT_MIDI_Receive_End
	call	MIDI_RxBufferGet	; get and parse incoming byte
	call	INT_MIDI_Parse
	movf	_int_midi_rx_package+0, W	; return first byte

INT_MIDI_Receive_End	
	return
...
;; --------------------------------------------------------------------------
INT_MIDI_Transmit
	call	MIDI_TxBufferFree	; exit if buffer not free
	addlw	-4			; exit if less than 4 bytes in buffer (to avoid blocking)
	bc	INT_MIDI_Transmit_Ok
	andlw	0x00
	rgoto	INT_MIDI_Transmit_End
INT_MIDI_Transmit_Ok
